...
export async function PUT(request: Request) {
  ...
  const existing = queryOne("SELECT id FROM hero WHERE id = @id", { id }) as { id: string } | undefined;
  ...
  if (existing) {
    ...
    const maxSort = queryOne("SELECT COALESCE(MAX(sort_order), -1) + 1 as next FROM hero") as { next: number };
    execute(
...
export async function DELETE(request: Request) {
  ...
  execute("DELETE FROM hero WHERE id = @id", { id });
  return NextResponse.json({ ok: true });
}
