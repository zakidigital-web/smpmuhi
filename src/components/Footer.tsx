...
function toUrl(input: string, base: string): string {
    ...
    <footer style={{ background: "var(--foreground)", color: "var(--text-secondary)", paddingBottom: "max(2rem, env(safe-area-inset-bottom, 0px))" }}>
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        <div className="grid grid-cols-1 md:grid-cols-4 gap-8">
          <div className="col-span-1 md:col-span-2">
            ...
            <div className="flex gap-4">
              <a href={toUrl(s.facebook, "https://facebook.com/")} target="_blank" rel="noopener noreferrer" className="w-10 h-10 rounded-full flex items-center justify-center transition-colors" style={{ background: "var(--card)", color: "var(--text-secondary)" }} aria-label="Facebook">
                <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24"><path d="M18 2h-3a5 5 0 0 0-5 5v3H7v4h3v8h4v-8h3l1-4h-4V7a1 1 0 0 1 1-1h3z" /></svg>
              </a>
              <a href={toUrl(s.instagram, "https://instagram.com/")} target="_blank" rel="noopener noreferrer" className="w-10 h-10 rounded-full flex items-center justify-center transition-colors" style={{ background: "var(--card)", color: "var(--text-secondary)" }} aria-label="Instagram">
              ...
              <a href={toUrl(s.youtube, "https://youtube.com/")} target="_blank" rel="noopener noreferrer" className="w-10 h-10 rounded-full flex items-center justify-center transition-colors" style={{ background: "var(--card)", color: "var(--text-secondary)" }} aria-label="YouTube">
        ...
        <div className="mt-8 pt-8 text-center" style={{ borderTop: "1px solid var(--card-border)", color: "var(--text-muted)" }}>
          <p>&copy; {new Date().getFullYear()} {s.schoolName}. {s.copyright}</p>
        </div>
      </div>
    </footer>
  );
